class King extends Piece {
  ...
  isValidMove(move, board) {
    ...
    if (
      abs(move.prevLoc[0] - move.curLoc[0]) > 1 ||
      ...
      if (this.rank == 1) {
        ...
        if (attempt == `bottomRight` && rightEnd.name == `Rook`) {
          if (rightEnd.moveCount == 0 && this.moveCount == 0) {
            if (
              board.getPiece(7, 1).name == `Empty` &&
              board.getPiece(6, 1).name == `Empty`
            ) {
              if (
                !board.isThreatened(5, 1, `Black`) &&
                !board.isThreatened(7, 1, `Black`) &&
                !board.isThreatened(6, 1, `Black`)
              ) {
                return true;
              }
            }
        ...
        if (attempt == `bottomLeft` && leftEnd.name == `Rook`) {
          if (leftEnd.moveCount == 0 && this.moveCount == 0) {
            if (
              board.getPiece(3, 1).name == `Empty` &&
              board.getPiece(2, 1).name == `Empty` &&
              board.getPiece(4, 1).name == `Empty`
            ) {
              if (
                !board.isThreatened(5, 1, `Black`) &&
                !board.isThreatened(4, 1, `Black`) &&
                !board.isThreatened(3, 1, `Black`)
              ) {
                return true;
              }
            }
    ...
    if (this.type == `Black`) {
      if (this.rank == 8) {
        ...
        if (attempt == `topRight` && rightEnd.name == `Rook`) {
          if (rightEnd.moveCount == 0 && this.moveCount == 0) {
            if (
              board.getPiece(7, 8).name == `Empty` &&
              board.getPiece(6, 8).name == `Empty`
            ) {
              if (
                !board.isThreatened(5, 8, `White`) &&
                !board.isThreatened(7, 8, `White`) &&
                !board.isThreatened(6, 8, `White`)
              ) {
                return true;
              }
            }
        ...
        if (attempt == `topLeft` && leftEnd.name == `Rook`) {
          if (leftEnd.moveCount == 0 && this.moveCount == 0) {
            if (
              board.getPiece(3, 8).name == `Empty` &&
              board.getPiece(2, 8).name == `Empty` &&
              board.getPiece(4, 8).name == `Empty`
            ) {
              if (
                !board.isThreatened(5, 1, `White`) &&
                !board.isThreatened(4, 1, `White`) &&
                !board.isThreatened(3, 1, `White`)
              ) {
                return true;
              }
            }
